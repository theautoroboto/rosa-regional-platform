...
locals {
  ...
  regional_check_proj_name = "mc-rchk-${local.resource_hash}"                    # 21 chars
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:logs:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.regional_dependency_check.name}",
          "arn:aws:logs:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:log-group:/aws/codebuild/${aws_codebuild_project.regional_dependency_check.name}:*",
      ...
      # These permissions allow Terraform to provision management cluster infrastructure
      {
        ...
        Action = [
          ...
          "logs:TagLogGroup",
          "logs:UntagLogGroup",
          # CodePipeline - For checking regional pipeline status
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          aws_codebuild_project.regional_dependency_check.arn,
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:codebuild:*:*:project/${aws_codebuild_project.regional_dependency_check.name}",
...
# CodeBuild Project - Regional Pipeline Dependency Check
resource "aws_codebuild_project" "regional_dependency_check" {
  name          = local.regional_check_proj_name
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "terraform/config/pipeline-management-cluster/buildspec-regional-dependency-check.yml"
  }
}
...
resource "aws_codepipeline" "regional_pipeline" {
  ...
  stage {
    name = "Wait-For-Regional-Maestro-IoT"

    action {
      name             = "CheckRegionalDependency"
      category         = "Build"
      owner            = "AWS"
      provider         = "CodeBuild"
      ...
      configuration = {
        ProjectName = aws_codebuild_project.regional_dependency_check.name
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      owner            = "AWS"
      provider         = "CodeBuild"
      input_artifacts  = ["dependency_check_output"]
